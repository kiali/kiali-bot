...
import {
    WebhookPayloadCheckRun, WebhookPayloadPullRequest, WebhookPayloadPullRequestPullRequest, WebhookPayloadStatus
...
export default class VersionBumpMerger extends Behavior {
  ...
  private checkRunCompletedHandler = async (context: Context<WebhookPayloadCheckRun>) => {
  ...
  private commitStatusChangedHandler = async (context: Context<WebhookPayloadStatus>) => {
  ...
  private getPrForCommit = async (api: GitHubAPI, repo: { owner: string, repo: string }, sha: string): Promise<PullsGetParams | null> => {
  ...
  private prCreatedHandler = async (context: Context<WebhookPayloadPullRequest>) => {
    ...
    context.log(VersionBumpMerger.LOG_FIELDS, `Adding comment about automatic merge to recently opened PR#${pull.number}`)
  ...
  private tryMergePr = async (api: GitHubAPI, pullParams: PullsGetParams) => {
    ...
    const okToMerge = this.isPrMergeable(pr) && (await this.isPrChecksOk(api, pr)) && (await this.isPrStatusOk(api, pr))
